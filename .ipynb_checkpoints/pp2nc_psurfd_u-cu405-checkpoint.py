...
filename1='umnsaa_psurfd000' # also what to call the filename out, and make a new directory, also the folder name
filename2='umnsaa_psurfd012'
fileoutname='umnsaa_psurfd'
...
filelisting2 = glob.glob(pathin + '**/' + filename2, recursive=True)

#filelisting1 = filelisting1[0:1] # just for testing
#filelisting2 = filelisting2[0:1] # just for testing
...
for x in range(len(filelisting1)):
    ...
    g=cf.read(filelisting2[x])
    f = cf.aggregate(f+g, dimension=('id',))
    long_name_function()
